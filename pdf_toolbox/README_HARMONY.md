...
- 通用工具详情页：文件/图片/TXT 选择、输入文件预览、输出文件名、动态参数、开始处理、进度、错误、成功结果、结果预览、保存、系统分享、历史记录。
- 记录页：读取历史、搜索/筛选、分页加载、结果预览、保存到系统文件位置、系统分享、删除、清空。
...
- 预览已提供统一 `FilePreviewPage`：图片可应用内直接查看，图片转 PDF/扫描 PDF 会按来源图片展示分页预览，TXT 可展示文本摘录，PDF/Office 等文件提供系统预览打开和保存兜底。分享已通过 `ShareKit` 的系统分享面板接入；保存已通过 `DocumentViewPicker.save` 写出到用户选择位置。
